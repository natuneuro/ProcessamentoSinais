...
%   necessário chamar sinalEEG{i}.evento.
    ...
    for i=1:length(sinalEEG)
        ...
        s_std = [std(cd1) std(cd2) std(cd3) std(cd4) std(cd5) std(ca5)];
        s_mean = [mean(cd1) mean(cd2) mean(cd3) mean(cd4) mean(cd5) mean(ca5)];
        s_max = [max(cd1) max(cd2) max(cd3) max(cd4) max(cd5) max(ca5)];
        s_min = [min(cd1) min(cd2) min(cd3) min(cd4) min(cd5) min(ca5)];
