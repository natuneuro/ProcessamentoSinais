...
def importar_evento():
    ...
    f_tse = filedialog.askopenfilename(
        title="Selecione o arquivo .tse", filetypes=(("tse files", "*.tse"), ("all files", "*.*"))
    )
    ...
    for i in range(0, quantidade_eventos - 1):
        evento = Evento.Evento(float(conteudo[i][0]), float(conteudo[i][1]), conteudo[i][2], float(conteudo[i][3]))
